...
namespace litehtml
{
	...
	void css::parse_import_rule(raw_rule::ptr rule, string baseurl, document::ptr doc, media_query_list_list::ptr media)
	{
		...
		doc->add_pending_stylesheet(
			container->import_css(url, baseurl, [=](const string& css_text, const string& new_baseurl) {
				parse_css_stylesheet(css_text, new_baseurl, doc, new_media, true);
			}));
	}
